...
# One-sim MC truth for CoES_tail_asset for ALL scenarios, using ONE U sample
# Uses your convention: scenario "a{cond}b{tail}"
# - cond  = conditioning threshold on distressed asset j (U_j <= cond)
# - tail  = ES cutoff level on the OTHER asset within that distress set
coes_truth_asset_from_U <- function(U_mc, scen = SCEN_COVAR, minN = 200L) {
  stopifnot(ncol(U_mc) == 2)
  
  parse_ab <- function(s) {
    a <- sub("^a([0-9.]+)b([0-9.]+)$", "\\1", s)
    b <- sub("^a([0-9.]+)b([0-9.]+)$", "\\2", s)
    c(a = as.numeric(a), b = as.numeric(b))
  }
  
  out <- matrix(NA_real_, nrow = 2, ncol = length(scen),
                dimnames = list(c("j1","j2"), scen))
  
  for (k in seq_along(scen)) {
    ab <- parse_ab(scen[k])
    
    alpha_cond <- ab["a"]   # distress probability level (conditioning)  <-- alpha in your setup
    beta_tail  <- ab["b"] #prob for CoES on the OTHER asset
    
    for (j in 1:2) {
      xx <- 3L - j
      
      idx <- which(U_mc[, j] <= alpha_cond)
      if (length(idx) < minN) next
      
      u_other <- U_mc[idx, xx]
      
      u_q <- as.numeric(stats::quantile(u_other, probs = beta_tail, names = FALSE))
      u_tail <- u_other[u_other <= u_q]
      
      out[j, k] <- mean(q_sstd(u_tail))
    }
  }
...
coes_rmse_mae_all <- function(out, dgp,
                              n_test = dim(out$CoES_tail_asset)[1],
                              scen_names = dimnames(out$CoES_tail_asset)[[3]],
                              map = NULL,
                              na.rm = TRUE) {
  
  if (is.null(out$CoES_tail_asset)) stop("out$CoES_tail_asset not found.")
  if (length(dim(out$CoES_tail_asset)) != 3) stop("out$CoES_tail_asset must be 3D: (time x cond_asset x scenario).")
  if (dim(out$CoES_tail_asset)[2] != 2) stop("Expected 2 conditioning assets in out$CoES_tail_asset.")
  if (is.null(scen_names)) stop("Scenario names missing: add dimnames(out$CoES_tail_asset)[[3]].")
  
  if (is.null(map)) {
    map <- list(
      a0.05b0.05  = "coes_true_a05b05",
      a0.05b0.1   = "coes_true_a05b10",
      a0.1b0.1    = "coes_true_a10b10",
      a0.1b0.05   = "coes_true_a10b05",
      a0.05b0.025 = "coes_true_a05b0025",
      a0.025b0.05 = "coes_true_a0025b05"
    )
  }
  ...
  for (sc in names(map)) {
    ...
    if (!(sc %in% scen_names)) stop(sprintf("Scenario '%s' not in out$CoES_tail_asset.", sc))
    if (is.null(dgp$truth[[truth_name]])) stop(sprintf("dgp$truth$%s not found.", truth_name))
    ...
    truth_last <- truth_mat[(nrow(truth_mat) - n_test + 1):nrow(truth_mat), , drop = FALSE]
    pred_slice <- out$CoES_tail_asset[, , sc, drop = FALSE][, , 1]
...
run_one_sim <- function(s,
  ...
  # --- Time series of CoES truth (innovation scale), efficiently ---
  L_truth <- 300000L
  minN    <- 200L
  
  coes_q_asset_t <- array(
    NA_real_,
    dim = c(Ttot, 2, length(SCEN_COVAR)),
    dimnames = list(NULL, c("j1","j2"), SCEN_COVAR)
  )
  
  for (g in dgp$regimes) {
    U_mc <- rvinecopulib::rvinecop(L_truth, g$vc)
    coes_reg <- coes_truth_asset_from_U(U_mc, scen = SCEN_COVAR, minN = minN)
    
    for (sc in SCEN_COVAR) {
      coes_q_asset_t[g$idx, , sc] <- matrix(coes_reg[, sc],
                                            nrow = length(g$idx),
                                            ncol = 2,
                                            byrow = TRUE)
    }
  ...
  scale_asset_target <- function(mat_j, sig_fc) {
    # mat_j: T x 2 (cols correspond to conditioning j=1,2)
    out <- mat_j
    out[, 1] <- out[, 1] * sqrt(sig_fc[, 2])  # j=1 -> target asset 2
    out[, 2] <- out[, 2] * sqrt(sig_fc[, 1])  # j=2 -> target asset 1
    out
  }
  coes_true_a05b05   <- scale_asset_target(coes_q_asset_t[, , "a0.05b0.05"],  data$sig_fc)
  coes_true_a05b10   <- scale_asset_target(coes_q_asset_t[, , "a0.05b0.1"],   data$sig_fc)
  coes_true_a10b10   <- scale_asset_target(coes_q_asset_t[, , "a0.1b0.1"],    data$sig_fc)
  coes_true_a10b05   <- scale_asset_target(coes_q_asset_t[, , "a0.1b0.05"],   data$sig_fc)
  coes_true_a05b0025 <- scale_asset_target(coes_q_asset_t[, , "a0.05b0.025"], data$sig_fc)
  coes_true_a0025b05 <- scale_asset_target(coes_q_asset_t[, , "a0.025b0.05"], data$sig_fc)
  ...
  dgp$truth <- list(
    ...
    coes_true_a05b05   = coes_true_a05b05,
    coes_true_a05b10   = coes_true_a05b10,
    coes_true_a10b10   = coes_true_a10b10,
    coes_true_a10b05   = coes_true_a10b05,
    coes_true_a05b0025 = coes_true_a05b0025,
    coes_true_a0025b05 = coes_true_a0025b05
  )
  ...
  rmse_mae_from_coes <- coes_rmse_mae_all(out, dgp)
  ...
  result <- list(
    ...
    rmse_mae_from_coes = rmse_mae_from_coes,
