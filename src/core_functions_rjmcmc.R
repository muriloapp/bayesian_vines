...
fast_vine_from_particle <- function(p, skel) {
  ...
  for (tr in seq_along(pcs)) {
    for (ed in seq_along(pcs[[tr]])) {
      ...
      if (p$gamma[idx] == 0L) {
        pcs[[tr]][[ed]] <- cfg$indep_copula
      } else if (!valid_tau(tau, fam_id)) {
...
#   else if (runif(1) < cfg$p_birth)  move <- "birth" 
#   else move <- "death"
#   
#   prop <- p            # deep copy
#   log_q_fwd  <- 0
...
# ────────────────────────────────────────────────────────────────────
#  rj_step()  –  two–stage move  (edge/θ  →  family)
# ────────────────────────────────────────────────────────────────────
rj_step <- function(p, data_up_to_t, skeleton, cfg) {
  
  choose_move <- function(n_on, K, cfg) {
    ## Feasibility mask
    can_birth <- n_on < K
    can_death <- n_on > 0
    probs <- c(within = cfg$p_within,
               birth  = if (can_birth) cfg$p_birth else 0,
               death  = if (can_death) cfg$p_death else 0)
  ...
  g      <- p$gamma
  n_on   <- sum(g)
  move   <- choose_move(n_on, cfg$K, cfg)        # << use helper
  ...
  if (move == "within") {
    ## random-walk on ONE active θ
    #e <- pick_edge(g == 1L)
    act <- which(prop$gamma == 1L)
    prop$theta[act] <- prop$theta[act] + rnorm(1, 0, cfg$step_sd)
  ...
  } else {             # death  (guaranteed at least one edge on)
    e               <- pick_edge(g == 1L)
    theta_old       <- prop$theta[e]
    prop$gamma[e]   <- 0L
    prop$theta[e]   <- 0
  ...
  ## =======  STAGE 2 :  optional family switch  =====================
  ##   – visit every ACTIVE edge
  ##   – with prob p_switch flip Gaussian ↔ Clayton (or any other)
  ##   – symmetric proposal → NO extra q terms
  active <- which(prop$gamma == 1L)
  if (length(active) && cfg$n_fam > 1L) {
    for (e in active) if (runif(1) < cfg$p_switch) {
      m_old <- prop$m[e]
      m_new <- sample(setdiff(seq_len(cfg$n_fam), m_old), 1)
      tau_e <- tanh(prop$theta[e])              # latent θ → Kendall τ
      if (valid_tau(tau_e, m_new))              # only flip if τ allowed
        prop$m[e] <- m_new
    }
  }
  ...
  ## prior = Normal on thetas that are ON  (no Bernoulli term)
  log_prior_prop <- sum(dnorm(prop$theta[prop$gamma == 1L],
                              0, cfg$slab_sd, log = TRUE))
  log_prior_curr <- sum(dnorm(p$theta[p$gamma == 1L],
                              0, cfg$slab_sd, log = TRUE))
  
  ## =======  Metropolis–Hastings ratio  =============================
  log_acc <- (ll_prop + log_prior_prop) - (ll_curr + log_prior_curr) +
    (log_q_back - log_q_fwd)        # family flip is symmetric
...
# ## ------------------------------------------------------------------
# ## tiny helper  – pick ONE index that satisfies a logical mask
# ## ------------------------------------------------------------------
# pick_edge <- function(mask) {
#   idx <- which(mask)
#   if (length(idx) == 0L) return(NA_integer_)
