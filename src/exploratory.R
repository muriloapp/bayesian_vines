...
plot_theta_paths(res_block$theta_mean, res_block$theta_se, k=1, theta_true = 0.6)
...
p <- plot_theta_paths_df(df_mean, df_se, theta_true=1, legend_labels = c("Tempering", "Standard"), plot_title = expression("Posterior path of " * gamma[12]))
ggsave(filename = "figures/static_3d_gamma12.png", plot = p, width = 8, height = 6)


df_ESS <- data.frame(
  block = res_block$diag_log$ESS,
  stand = res_stand$diag_log$ESS,
  threshold = rep(500, length( res_stand$diag_log$ESS))
)

p <- plot_theta_paths_df(df_ESS,  plot_title = "Effective Sample Size (Hits: 17 for standard and 22 for blocks)")
ggsave(filename = "figures/static_3d_ESS.png", plot = p, width = 8, height = 6)
...
ggsave(filename = "figures/static_3d_unique.png", plot = p, width = 8, height = 6)
