...
run_empirical <- function() {
  ...
  dat <- list(
    U      = readRDS("data/PIT.rds")[1:1500,1:3],
    mu_fc  = readRDS("data/returns_mean_forecast.rds")[1:1500,2:4],# [,-1],  # drop date col
    sig_fc = readRDS("data/returns_vol_forecast.rds")[1:1500,2:4],  #[,-1],
    df_fc = readRDS("data/df_fc.rds")[1:1500,2:4]#[,-1]
  )
